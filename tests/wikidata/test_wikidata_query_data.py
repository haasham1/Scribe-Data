        ...
        with patch("sys.stdout", new=StringIO()) as out:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with (
                    ...
                    patch(
                        "scribe_data.wikidata.query_data.list_all_languages",
                        return_value=["German"],
                    ),
                    patch(
                        "scribe_data.wikidata.query_data.format_sublanguage_name",
                        return_value="German",
                    ),
                    patch(
                        "scribe_data.wikidata.query_data.sparql.setQuery"
                    ) as mock_setQuery,
                    patch("scribe_data.wikidata.query_data.sparql.query") as mock_query,
                    patch(
                        "scribe_data.wikidata.query_data.execute_formatting_script"
                    ) as mock_exec,
                ):
                    ...
                    self.assertIn(
                        "Querying and formatting German verbs", out.getvalue()
                    )
                    self.assertIn(
                        "Successfully queried and formatted data for German verbs",
                        out.getvalue(),
                    )
        ...
        with patch("sys.stdout", new=StringIO()) as out:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with (
                    ...
                    # Check the expected messages are printed to sys.stdout.
                    self.assertEqual(
                        out.getvalue().count("Querying and formatting German verbs"), 3
                    )
                    self.assertEqual(
                        out.getvalue().count(
                            f"Nothing returned by the WDQS server for {query_file}"
                        ),
                        3,
                    )
                    self.assertEqual(
                        out.getvalue().count("The query will be retried."), 2
                    )
                    self.assertIn(
                        "Max retries reached. Skipping this query.", out.getvalue()
                    )
        ...
        with patch("sys.stdout", new=StringIO()) as out:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with (
                    ...
                    # Simulate query responses.
                    mock_http = MagicMock()
                    mock_http.convert.side_effect = HTTPError(
                        "url", 404, "error", None, None
                    )
                    mock_query.side_effect = [
                        MagicMock(
                            convert=lambda: {
                    ...
                    # Check the expected messages are printed to sys.stdout.
                    self.assertEqual(
                        out.getvalue().count("Querying and formatting German verbs"), 4
                    )
                    self.assertEqual(
                        out.getvalue().count(
                            f"HTTPError with {query_file_2}: HTTP Error 404: error"
                        ),
                        1,
                    )
                    self.assertEqual(
                        out.getvalue().count(
                            f"Nothing returned by the WDQS server for {query_file_2}"
                        ),
                        4,
                    )
                    self.assertEqual(
                    ...
                    self.assertEqual(
                        out.getvalue().count(
                            "Max retries reached. Skipping this query."
                        ),
                        1,
                    )
