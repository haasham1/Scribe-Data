...
def validate_forms(query_text: str) -> str:
    ...
    for match in re.finditer(forms_pattern, query_text):
        where_vars.add(f"?{match.group(1)}")
    ...
    for match in re.finditer(triple_pattern, query_text):
        where_vars.add(f"?{match.group(1)}")
        where_vars.add(f"?{match.group(2)}")
    ...
    if undefined := [var for var in select_vars if var not in where_vars]:
        errors.append(
    ...
    if unreturned := [var for var in form_vars if var not in select_vars]:
        errors.append(
